...
class MemoryEfficientReplayBuffer(ReplayBuffer):
    ...
    def sample(
        ...
        batch = super().sample(batch_size, keys, indx, sample_futures=False, relabel=False)
        batch = batch.unfreeze()

        def add_observation(indx, obs_keys, add_next_observation=False):
            target_dict = {}
            next_obs_dict = {}
            ...
            for k in obs_keys:
                target_dict[k] = _sample(
                    self.dataset_dict["observations"][k], indx
                )
                if add_next_observation:
                    next_obs_dict[k] = _sample(
                        self.dataset_dict["next_observations"][k], indx
                    )
            ...
            for pixel_key in self.pixel_keys:
                ...
                if pack_obs_and_next_obs:
                    target_dict[pixel_key] = obs_pixels
                else:
                    target_dict[pixel_key] = obs_pixels[..., :-1]
                    if add_next_observation:
                        next_obs_dict[pixel_key] = obs_pixels[..., 1:]

            return target_dict, next_obs_dict

        obs_keys = self.dataset_dict["observations"].keys()
        batch["observations"], batch["next_observations"] = add_observation(indx, obs_keys, add_next_observation=True)

        if sample_futures:
            future_indices = self.calculate_future_observation_indices(indx, sample_futures)
            batch["future_observations"], _ = add_observation(future_indices, obs_keys, add_next_observation=False)
