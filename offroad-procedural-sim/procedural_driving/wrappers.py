        ...
        return {
            **observation,
            "states": np.concatenate(
                [observation[k].reshape(-1) for k in self.keys], axis=0
            ),
        }
    ...
    def step(self, action):
        ...
        if len(step_returns) == 5:
            ...
            return (
                ...
                terminated,
                truncated,
                {**info, "running_return": self._running_return},
            )
        else:
            raise ValueError("Old step_api not supported")
...
class ReplaceKey(gym.ObservationWrapper):
    ...
    def __init__(
        self, env: gym.Env, key: str, new_key: str
    ):
        super().__init__(env)
        self._key = key
